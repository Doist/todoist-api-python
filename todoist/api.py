...
from todoist.http_client import delete, get, post
...
class TodoistAPI:
    ...
    def get_tasks(self, **kwargs) -> List[Task]:
        ...
        tasks = get(self._token, endpoint, kwargs)
    ...
    def add_task(self, content: str, **kwargs) -> Task:
        ...
        data.update(kwargs)
        task = post(self._token, endpoint, data=data)
        return Task.from_dict(task)
    ...
    def update_task(self, task_id: str, **kwargs) -> Task:
        endpoint = get_rest_url(f"{TASKS_ENDPOINT}/{task_id}")
        task = post(self._token, endpoint, data=kwargs)
        return Task.from_dict(task)
    ...
    def close_task(self, task_id: str) -> bool:
        ...
        success = post(self._token, endpoint)
        return success
    ...
    def delete_task(self, task_id: str) -> bool:
        endpoint = get_rest_url(f"{TASKS_ENDPOINT}/{task_id}")
        success = delete(self._token, endpoint)
        return success
